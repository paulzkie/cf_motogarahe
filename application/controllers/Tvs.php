...
class Tvs extends CI_Controller {
	...
	{
		$slug = $this->sanitize_plain_text(isset($post_data['mot_slug']) ? $post_data['mot_slug'] : $fallback_slug, 120);
		$slug = ($slug !== '') ? $slug : $fallback_slug;

		return array(
			'mot_slug' => $slug,
			'dea_name' => $this->sanitize_plain_text(isset($post_data['dea_name']) ? $post_data['dea_name'] : 'dealer', 120),
			...
			'loc' => strtolower($this->sanitize_plain_text(isset($post_data['loc']) ? $post_data['loc'] : '', 40)),
		);
	}
	...
	public function index($slug="all", $brand="brand", $type="type", $transmission="transmission", $diplacement="diplacement", $engine="engine-type", $filter="1" )
	{
		...
		$this->load->view("tvs/template/site_header", $header);
		$this->load->view('tvs/site/view_tvs_motorcycles', $content);
		$this->load->view("tvs/template/site_footer", $footer);
	}
	...
	public function info($slug) {
		...
		if($this->input->post('dealers_mode')) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				redirect('tvs/dealers/' . $slug . '/' . $data['dem_colors'] . "/" . $data['lat'] . '/' . $data['long'] . '/' . 100 . '/' . 0 . '/' . 0 . '/' . 'dealer' . '/' . 0 . '/' . 0  ,'refresh');
			}
		}
		...
		$this->load->view("tvs/template/site_header", $header);
		$this->load->view('tvs/site/view_tvs_info', $content);
		$this->load->view("tvs/template/site_footer");	
		...
		if($this->input->post('fill_mode')) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				**/
				...
				$this->session->set_flashdata('msg_success', 'Inquiry sent Successfully !');
				redirect('tvs/guide' ,'refresh');
				
			}
		}

		$this->load->view("tvs/template/site_header", $header);
		$this->load->view('tvs/site/view_tvs_inquiry', $content);
		$this->load->view("tvs/template/site_footer");	
	...
	public function dealers($slug, $dem_colors, $lat, $long, $km, $loc_lat, $loc_long, $dealer, $minprice, $maxprice, $filter="1")
	{
		...
		if($this->input->post('search_mode')) {
			...
			$this->form_validation->set_rules('loc', 'Location', 'trim|max_length[40]');
		...
		// $this->load->view("template/site_header", $header);
		$this->load->view("tvs/template/site_header", $header);
		$this->load->view('tvs/site/view_tvs_dealers', $content);
		$this->load->view("tvs/template/site_footer", $footer);
		...
		foreach($data as $motorcycle)
			{
				...
				<a href=" '.base_url('tvs/info/') . '/' . $motorcycle['mot_slug'].' ">
				...
				<h4 class="moto-descrip">₱'.number_format( $motorcycle['mot_srp'], 2);
	...
	public function mall()
	{
       // $header = [];
		...
		$header = array(
			'header_title' => 'Motorcycles',
			'header_desc' => 'Motogarahe.com is an interactive website that helps you to search, compare and purchase the right motorcycle for you.',
			'header_keywords' => "",
			'header_featured_img' => "",
			'mot_model' => ""
		);
        ...
        $this->load->view("tvs/template/site_header", $header);
		$this->load->view('tvs/site/view_tvs_mall', $content);
		$this->load->view("tvs/template/site_footer", $footer);
	}
	public function guide(){
		...
		$this->load->view("tvs/template/site_header", $header);
		$this->load->view('tvs/site/view_tvs_guide', $content);
		$this->load->view("tvs/template/site_footer", $footer);
	}
	...
	public function slide(){
		$content = [];
		$this->load->view("tvs/template/site_header", $header);
		$this->load->view('tvs/site/slider', $content);
		$this->load->view("tvs/template/site_footer", $footer);

	}
	public function voucher_counter(){

		if($this->input->post("download")){
			$data = $this->input->post();
			unset($data['download']);
			$data["voucher_date"] = $this->getDatetimeNow();
			$this->model_base->insert_data($data_users, 'voucher');
			$this->session->set_flashdata('msg_success', 'Downloading voucher please wait ....');
			redirect('tvs','refresh');
		}
	}
