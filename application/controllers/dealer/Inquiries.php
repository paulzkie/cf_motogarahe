...
class Inquiries extends CI_Controller {
    ...
    public function index( $inq_name = "xallx", $inq_created_from = "xallx", $inq_created_to = "xallx", $filter = 1 )
	{
		...
		if ( $this->input->post('search_mode') ) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				// $this->godprintp($data);
				redirect('dealer/inquiries/index/' . $inq_name ."/" . $inq_created_from . "/". $inq_created_to, 'refresh');
			}	
		}

		$config = array();
		$config["base_url"] = base_url() . "dealer/inquiries/index/". $inq_name ."/" . $inq_created_from . "/". $inq_created_to ."/";
		...
		$content['all'] = base_url('dealer/inquiries/index/xallx/xallx');
		$content['published'] = base_url('dealer/inquiries/index/inq_status/published');
		$content['draft'] = base_url('dealer/inquiries/index/inq_status/draft');
		$content['deleted'] = base_url('dealer/inquiries/index/inq_status/deleted');
		$content['create'] = base_url('dealer/inquiries/create');

		$this->load->view("template/dealer_header", $header);
		$this->load->view('dealer/inquiries/index', $content);
    ...
    public function _sort ($inq_name, $inq_created_from, $inq_created_to) {
		...
		if ( $this->session->userdata('acc_head_office') === 'true' ) {
			$this->db->where('inquiries_new.dea_id', $this->session->userdata('dea_id'));
			$this->db->join("motorcycles", "motorcycles.mot_id = inquiries_new.mot_id");
			$this->db->join("dealers_branches", "dealers_branches.deb_id = inquiries_new.deb_id");
		} else {
    ...
    public function edit($id)
	{
		...
		if($this->input->post()) {
			...
			$this->form_validation->set_rules('inq_remarks', 'Inquiry Remarks', 'trim');
			...
			if ($this->form_validation->run() == FALSE) {
				...
				redirect('dealer/inquiries/edit/' . $id, 'refresh');
			}
		}
		...
		$this->load->view("template/dealer_header", $header);
		$this->load->view('dealer/inquiries/edit', $content);
		$this->load->view("template/dealer_footer");
	}
