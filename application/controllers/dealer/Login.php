...
class Login extends CI_Controller {
	...
	public function index()
	{
		if ( $this->have_sess_branch() == true ){
			...
			if($this->input->post()) {
				$login_limit = $this->request_throttle->hit_login_buckets('dealer_login', $this->input->ip_address(), $this->input->post('acc_username', TRUE));
				...
				if ($this->form_validation->run() == FALSE) {
	    			...
	    			if ( count( $account ) >= 1 ) {
		    			$this->request_throttle->clear_login_buckets('dealer_login', $this->input->ip_address(), $this->input->post('acc_username', TRUE));
