...
class motorcycles_royalenfield extends CI_Controller {
	...
	public function index($slug="")
	{
		...
		$header['header_title'] = $content['motorcycles'][0]['mot_brand'].' '. $content['motorcycles'][0]['mot_model'] ." 2021 Philippines Price and Specs | Motogarahe.com". " - ₱" . number_format( $content['motorcycles'][0]['mot_srp'], 2);
...
// 	}
		...
		if($this->input->post('fill_mode')) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				**/
				...
				$bajajmodels = array('re-4s','re-4s-new','maxima-z');
	...
	public function loadroyalenfieldmotor(){
		$output = '';
		$limit = intval($this->clean_input($this->input->post("limit")));
		$offset = intval($this->clean_input($this->input->post("offset")));
		// $slug  = $this->input->post("slug") == 'all' ? 'all': $this->clean_input($this->input->post("slug")) ;
