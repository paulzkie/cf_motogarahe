...
class Motorcycles extends CI_Controller {
	...
	public function _ajaxsort($slug, $brand, $type, $transmission, $diplacement, $engine, $sort){
		...
		// if(is_array($sort)){
		// 	if(in_array(1,$sort)){
		// 		$this->db->get("motorcycles");
		// 	}
		// }
		// if(is_array($sort)){
		// 	if(count($sort) <=0 ){
		// 	}else{
		// 		$this->db->order_by('mot_id ASC');
		// 		// foreach($sort as $val){
	...
	public function search_suggestion_nav(){
		...
		foreach($allmotors as $result){
			...
			echo  '<li class="list-group-item moto-result-nav-input-search"  onmouseover="hoverInResNav(event)" onmouseoutNav="hoverOutResNav(event)" onclick="pickResultNav('.$values.')" >' .$result["mot_brand"] . " ".$result["mot_model"] . " </li>"; 
			
		}

	}
	public function search_nav_page(){
