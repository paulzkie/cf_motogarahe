...
class motorcyclespromogms extends CI_Controller {
	...
	public function index($slug="")
	{
		...
		$header = [];
		$content = [];
		$header['header_title'] = 'GMS Promo';
		$header['header_desc'] = "Motogarahe GMS Promo.";
		...
		if($brand!=strtolower($content['motorcycles'][0]['mot_brand'])){
			...
			redirect('motorcyclespromogms'.'/'.strtolower($content['motorcycles'][0]['mot_brand']).'/'. $slug,'refresh');
		}
		}else{
		...
		$header['header_title'] = $content['motorcycles'][0]['mot_brand'].' '. $content['motorcycles'][0]['mot_model'] ." 2021 Philippines Price and Specs | Motogarahe.com". " - ₱" . number_format( $content['motorcycles'][0]['mot_srp'], 2)." GMS Promo";
        $header['header_desc'] = "Buy your ".$content['motorcycles'][0]['mot_keyword']." 2021 GMS Promo for the price of ₱".$content['motorcycles'][0]['mot_srp'].". through The #1 Online Motorcycle Marketplace in the Philippines and experience an easier, safer, and â€œsulitâ€ way of buying the perfect ride for you.";
		...
		$this->load->view("newui/template/site_header", $header);
		$this->load->view('newui/site/moto-infopromogms', $content);
	...
	public function loadmotorpromogms(){
		$output = '';
		$limit = intval($this->clean_input($this->input->post("limit")));
		$offset = intval($this->clean_input($this->input->post("offset")));
		// $slug  = $this->input->post("slug") == 'all' ? 'all': $this->clean_input($this->input->post("slug")) ;
		...
		$this->db->where('mot_status', 'published');
		$this->db->where('mot_promo', 'true');
		$this->db->where('mot_promoname', 'gms');
		$data = $this->model_base->get_all('motorcycles');
		$count= 0;
		// echo print_r($displacement);
		foreach($data as $motorcycle)
			{
				$output .= '<a href=" '.base_url().'motorcycles'.'/'.strtolower($motorcycle['mot_brand']). '/' . $motorcycle['mot_slug'].' "></a>
				<div data-count="'.$count++.'" class="col-lg-4 col-md-6 col-sm-6 col-6 moto-col mot_result">
				<div class="data-holder "><h5 style="color:red;">ON SALE!</h5>
				<div class="holder"><div class="parallelogram d-none" >P 1000 <span class="off-text">Off</span></div></div>
				<a href=" '.base_url().'motorcyclespromogms'.'/'.strtolower($motorcycle['mot_brand']). '/' . $motorcycle['mot_slug'].' ">
				<img class="moto-img" src=" '.base_url(). $motorcycle['mop_image'] .'" alt="'. $motorcycle['mot_model'] .'" 
				title="'.$motorcycle['mot_brand'].' '.$motorcycle['mot_slug']. ' 2021" width="300" height=""></a>
				<div class="holder"><div class="promo-shape"></div></div>
				<h3 class="moto-title" style="height: auto;">'. $motorcycle['mot_brand'].' '. $motorcycle['mot_model'] .'</h3>
				<h4 class="moto-descrip"><span style="color:red;">Zero Downpayment</span></del></h4>';
