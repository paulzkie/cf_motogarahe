...
class Logs extends CI_Controller {
	...
	{
		$header = [];
		$header['header_title'] = 'Admin Logs';
		$header['header'] = 'Admin Logs';
		$header['header_small'] = '';
		...
		if ( $this->input->post('search_mode') ) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				redirect('admin/logs/index/xallx/xallx', 'refresh');
			} else {
				...
				// $this->godprintp($data);
				redirect('admin/logs/index/' . $search_type . '/' . $search_val, 'refresh');
			}	
		}
		...
		$config = array();
		$config["base_url"] = base_url() . "admin/logs/index/". $search_type ."/". $search_val ."/";
		//$this->_sort($search_type, $search_val );
		$total_row = $this->model_base->count_data('logs');
		...
		$this->db->join('accounts b','a.userid=b.acc_id');
		$content['logs'] = $this->model_base->get_all('logs a');
        
        $logdata = array(
                'userid' => $this->session->userdata('acc_id'),
                'logdesc' => 'View admin logs',
                'logdate' => $this->getDatetimeNow()
            );
            
        $this->model_base->insert_data($logdata,'logs');
        
		$this->load->view("template/admin_header", $header);
		$this->load->view('admin/logs/index', $content);
