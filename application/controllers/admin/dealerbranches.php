...
class dealerbranches extends CI_Controller {
    ...
    public function dealers($search_type = "xallx", $search_val ="xallx", $filter = 1 )
	{
		$header = [];
		$header['header_title'] = 'Dealers';
		$header['header'] = 'Dealers';
		$header['header_small'] = '';
        ...
        if ( $this->input->post('search_mode') ) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				redirect('admin/dealerbranches/dealers/xallx/xallx', 'refresh');
			} else {
				...
				// $this->godprintp($data);
				redirect('admin/dealerbranches/dealers/' . $search_type . '/' . $search_val, 'refresh');
			}	
		}	
		

		$config = array();
		$config["base_url"] = base_url() . "admin/dealerbranches/dealers/". $search_type ."/". $search_val ."/";
		//$this->_sort($search_type, $search_val );
		$total_row = $this->model_base->count_data('dealers');
		...
		if($this->input->get('search_val')==''){
		    $this->db->order_by('dea_id', 'DESC');
    		$content['dealers'] = $this->model_base->get_all('dealers');
		}else{
		    $this->db->like('dea_name', $this->input->get('search_val'));  
		    $content['dealers'] = $this->model_base->get_all('dealers');
		}
		...
		$content['all'] = base_url('admin/dealerbranches/dealers/xallx/xallx');
		$content['published'] = base_url('admin/dealerbranches/dealers/mot_status/published');
		$content['draft'] = base_url('admin/dealerbranches/dealers/mot_status/draft');
		$content['deleted'] = base_url('admin/dealerbranches/dealers/mot_status/deleted');
		$content['create'] = base_url('admin/dealerbranches/create');
    ...
    public function branches($search_type = "xallx", $search_val ="xallx", $filter = 1 ){
        $header = [];
		$header['header_title'] = 'Branches';
        ...
        if ( $this->input->post('search_mode') ) {
			...
			if ($this->form_validation->run() == FALSE) {
				$this->session->set_flashdata('msg_error', validation_errors());	
				redirect('admin/dealerbranches/branches/xallx/xallx', 'refresh');
			} else {
				$data = $this->input->post();
				...
				$search_type = $data['search_type'];
				// $this->godprintp($data);
				redirect('admin/dealerbranches/branches/' . $search_type . '/' . $search_val, 'refresh');
			}	
		}	
		

		$config = array();
		$config["base_url"] = base_url() . "admin/dealerbranches/branches/". $search_type ."/". $search_val ."/";
		//$this->_sort($search_type, $search_val );
		$this->db->join('dealers b','a.dea_id=b.dea_id');
		$total_row = $this->model_base->count_data('dealers_branches a');
		$config["total_rows"] = $total_row;
		$config['per_page'] = 20;
		$config['uri_segment'] = 6;
		$config['num_links'] = $total_row;
		...
		//$this->_sort($search_type, $search_val );
		if($this->input->get('search_val')==''){
    		$this->db->order_by('deb_id', 'DESC');
    		$this->db->join('dealers b','a.dea_id=b.dea_id');
    		$content['branches'] = $this->model_base->get_all('dealers_branches a');
    
		}else{
		    $this->db->like('acc_name', $this->input->get('search_val'),'both');
		    $this->db->or_like('acc_username', $this->input->get('search_val'),'both');
		    $this->db->join('dealers b','a.dea_id=b.dea_id');
		    $content['branches'] = $this->model_base->get_all('dealers_branches a');
		}
        
        $content['controller']=$this; 
		$content['all'] = base_url('admin/dealerbranches/branches/xallx/xallx');
		$content['published'] = base_url('admin/dealerbranches/branches/mot_status/published');
		$content['draft'] = base_url('admin/dealerbranches/branches/mot_status/draft');
		$content['deleted'] = base_url('admin/dealerbranches/branches/mot_status/deleted');
		$content['create'] = base_url('admin/dealerbranches/create');


		$this->load->view("template/admin_header", $header);
		$this->load->view("template/popupmsg");
		$this->load->view('admin/branches/branchindex', $content);
		$this->load->view("template/admin_footer");
    }
    
    public function updatedealer(){
        $updatedata = array(
                'dea_name' => $this->input->post('updatedea_name'),
                'dea_slug' => $this->input->post('updatedea_slug'),
                'dea_email' => $this->input->post('updatedea_email'),
                'dea_status' => $this->input->post('dea_status')
            );
        $this->model_base->update_data($this->input->post('dea_id'),'dea_id',$updatedata,'dealers');
    ...
    public function updatebranch(){
        $updatedata = array(
                ...
                'cp' => $this->input->post('updatecp'),
                'repo_slot' => $this->input->post('updateslot'),
                'deb_status' => $this->input->post('deb_status')
            );
        $this->model_base->update_data($this->input->post('deb_id'),'deb_id',$updatedata,'dealers_branches');
        
        echo 'Data Successfully updated!';
    }
    
    public function updatebranchall(){
        $updatedata = array(
                'cp' => $this->input->post('updatecp'),
                'repo_slot' => $this->input->post('updateslot')
            );
        $this->model_base->update_data($this->input->post('dea_id'),'dea_id',$updatedata,'dealers_branches');
	...
	public function create()
	{
		...
		$this->form_validation->set_rules('mot_battery', '', 'trim');
		...
		$this->form_validation->set_rules('mot_range', '', 'trim');
		...
		$this->form_validation->set_rules('mot_alarm', '', 'trim');
		...
		if($this->input->post()) {
		  ...
		  //       } 
				...
				$this->session->set_flashdata('msg_success', 'test!');	
				redirect('admin/motorcycles','refresh');
	...
	public function view($id)
	{
		...
		if($this->input->post("fb_pic")) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$ImageCount = count($_FILES['mot_fb_img']['name']);
				...
				if ( $ImageCount == 1) {
					$filesCount = $ImageCount;
		            for($i = 0; $i < $filesCount; $i++){
		                $_FILES['file']['name']     = $_FILES['mot_fb_img']['name'][$i];
		                $_FILES['file']['type']     = $_FILES['mot_fb_img']['type'][$i];
		                $_FILES['file']['tmp_name'] = $_FILES['mot_fb_img']['tmp_name'][$i];
		                $_FILES['file']['error']     = $_FILES['mot_fb_img']['error'][$i];
		                $_FILES['file']['size']     = $_FILES['mot_fb_img']['size'][$i];
		                ...
		                if($this->upload->do_upload('file')){
		                    ...
		                    $fileData = $this->upload->data();
		                    // $this->godprint($fileData);
							$data['mot_fb_img'] = 'uploads/motorcycles/' . $fileData['file_name'];
							$update_data = array ('mot_fb_img' => $data['mot_fb_img']);
		                    // $uploadData[$i]['uploaded_on'] = date("Y-m-d H:i:s");
							// $this->godprint($data);
							$col = 'mot_id';
							$tbname = 'motorcycles';
		                    // $this->model_base->update_data($data["mot_id"],$col,$data['mot_fb_img'], $tbname);
		                    $this->model_base->update_data($data["mot_id"],$col,$update_data, $tbname);
		                }
		            }
					...
					if ( $ImageCount >= 2) {
						$this->session->set_flashdata('msg_error', 'One Photo Only!');	
	...
	public function delete_picture_fb($mot_id) {
		
		$content['motorcycles'] = $this->model_base->delete_data($mot_id, 'mot_id', 'mot_fb_img', 'motorcycles');
