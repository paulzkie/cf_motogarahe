...
class Home extends CI_Controller {
	...
	public function index()
	{
		...
		$header['testimonial'] = $this->model_base->get_all('homepage_testimonial');
	 ...
	 //    if ( $this->session->userdata('uss_id') != 0 ) {
		...
		$this->load->view('newui/site/moto-home', $header);
	...
	public function subscriptions(){
		$data = [];
		$data["email_add"] = $this->input->post("EMAIL");
		$data["created_date"] = $this->getDatetimeNow();
		$this->model_base->insert_data($data, 'subscription');
		$this->db->flush_cache();

		$this->session->set_flashdata('msg_success', 'Successfully Subscribe!');
   ...
   {	
	...
	public function home_dev()
	{
		...
		if($this->input->post('search_mode')) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ( $data['status'] == "NEW") {
					...
					redirect('motorcycles/index_dev/' . $data['mot_model'] . '/' . $data['mot_brand'] . '/' . $data['mot_type'] . '/' . $data['mot_transmission'] . '/' . $data['mot_diplacement'] . '/' . $data['mot_engine_type'] ,'refresh');
				} else {
					redirect('repo/index_dev/' . $data['mot_model'] . '/' . $data['mot_brand'] . '/' . $data['mot_type'] . '/' . $data['mot_transmission'] . '/' . $data['mot_diplacement'] . '/' . $data['mot_engine_type'] ,'refresh');
				}
			}
		}
	 ...
	 //    if ( $this->session->userdata('uss_id') != 0 ) {
		...
		$this->load->view("newui/template/site_header", $header);
		$this->load->view('newui/site/moto-home-dev', $content);
		$this->load->view("newui/template/site_footer", $footer);
