...
class Ebike extends CI_Controller {
	...
	{
		return public_input_plain_text($value, $max_length);
	}
	...
	{
		return public_input_phone($value);
	}

	private function sanitize_email_value($value)
	{
		return public_input_email($value);
	}
	...
	{
		return public_input_registration_payload($post_data);
	}

	private function sanitize_login_payload($post_data)
	{
		return public_input_login_payload($post_data);
	}
	...
	private function sanitize_search_redirect_payload($post_data)
	{
		$build_segment = function ($key, $default, $max_length = 80) use ($post_data) {
			$value = $this->sanitize_plain_text(isset($post_data[$key]) ? $post_data[$key] : '', $max_length);
		...
		$status = strtoupper($this->sanitize_plain_text(isset($post_data['status']) ? $post_data['status'] : '', 10));
		if ($status !== 'USED') {
			$status = 'NEW';
		}
	...
	private function build_inquiry_email_message($data, $dealer_motorcycle, $selected_color)
	{
		...
		return 'Dear. Mr./Ms. ' . html_escape(ucwords($data['inq_name'])) . ' <br/><style>.im{color: black !important;}</style><p style="color: black !important;"> Thank you for using motogarahe.com in buying your dream motorcycle. You’re a few steps away to complete your purchase. <br/> Our Partner-dealer will call you within 24hrs to guide you in your buying journey.</p><table>' . $table_rows . '</table><br/><img style="background: black;" height="50px" with="300px" src="https://www.motogarahe.com/uploads/icon/new-hanapmototag.png"/><br/><br/><img style="background: white;" height="500px" width="1040px" src="https://www.motogarahe.com/uploads/guide/mailimgv2.png"/>';
	}
	...
	public function index($slug="")
	{
		...
		$header = [];
		$content = [];
		$header['header_title'] = 'Motorcycles | Ebike';
		$header['header_desc'] = "Check different e-bikes brands and models that are available here in the Philippines.";
		...
		if($brand!=strtolower($content['motorcycles'][0]['mot_brand'])){
			...
			redirect('motorcycles'.'/'.strtolower($content['motorcycles'][0]['mot_brand']).'/'. $slug,'refresh');
		}
		}else{
		...
		$header['header_title'] = $content['motorcycles'][0]['mot_brand'].' '. $content['motorcycles'][0]['mot_model'] ." 2021 Philippines Price and Specs | Motogarahe.com". " - ₱" . number_format( $content['motorcycles'][0]['mot_srp'], 2)." | Ebike";
	...
	// public function search($slug="all", $brand="brand", $type="type", $transmission="transmission", $diplacement="diplacement", $engine="engine-type", $filter="1" )
	// {
...
// 		// Buy your Honda Wave 110 Alpha 2020 for the price of ₱47,000.00 through The #1 Online Motorcycle Marketplace in the Philippines and experience an easier, safer, and â€œsulitâ€ way of buying the perfect ride for you.
// 		//$header['header_desc'] = $content['motorcycles'][0]['mot_desc'] != '' ? $content['motorcycles'][0]['mot_desc'] : "Buy ".$content['motorcycles'][0]['mot_keyword']." online at motogarahe.com. Discount and Promotions sale on all motors.";
// 		$header['header_title'] = $content['motorcycles'][0]['mot_model'] ." 2020 Philippines Price and Specs | Motogarahe.com". " - ₱" . number_format( $content['motorcycles'][0]['mot_srp'], 2);
//         $header['header_desc'] = $content['motorcycles'][0]['mot_desc'] != '' ? $content['motorcycles'][0]['mot_desc'] : "Buy your ".$content['motorcycles'][0]['mot_keyword']." 2020 for the price of ₱".$content['motorcycles'][0]['mot_srp'].". through The #1 Online Motorcycle Marketplace in the Philippines and experience an easier, safer, and â€œsulitâ€ way of buying the perfect ride for you.";
...
// 	}
			...
			if ($this->form_validation->run() == FALSE) {
				...
				foreach ($this->session->userdata('selected_dealers') as $dealer) {
					...
					if($this->uri->segment(5)=='promogms'){
    					$data['inq_promo_remarks'] = 'GMS PROMO';
    					$this->model_base->insert_data($data, 'inquiries_new');	
					}else if($this->uri->segment(5)=='promo'){
    					$data['inq_promo_remarks'] = 'MOTORCENTRAL PROMO';
    					$this->model_base->insert_data($data, 'inquiries_new');    
					}else{
					    $this->model_base->insert_data($data, 'inquiries_new');	
					}
				...
				**/
				...
				if(in_array($this->uri->segment(3), $bajajmodels)){
					redirect('guide/'.$this->uri->segment(3) ,'refresh');
				}else{
					redirect('guide' ,'refresh');	
				}
			}
	...
	public function dealers($slug, $dem_colors, $lat, $long, $km, $loc_lat, $loc_long, $dealer, $minprice, $maxprice,$promo , $filter="1")
	{
		...
		//$content['promo'] = $this->clean_input($this->uri->segment(13));
		...
		$content['motorcycles'] = $this->_sort_dealers($slug, $dem_colors, $lat, $long, $km, $loc_lat, $loc_long, $dealer, $minprice, $maxprice, $this->uri->segment(13));
		...
		if($this->input->post('reg_mode')) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$data = $this->sanitize_registration_payload($this->input->post(NULL, FALSE));
		...
		if($this->input->post('login_mode')) {
			...
			$this->form_validation->set_rules('usr_email', 'Email', 'required|trim|valid_email|max_length[254]');
			$this->form_validation->set_rules('usr_password', 'Password', 'required|trim');
		...
		if($this->input->post('search_mode')) {
			...
			$this->form_validation->set_rules('mot_slug', 'Motorcycle', 'trim|max_length[120]');
			$this->form_validation->set_rules('dea_name', 'Dealer', 'trim|max_length[120]');
			$this->form_validation->set_rules('km', 'Distance', 'trim|integer');
			$this->form_validation->set_rules('minprice', 'Minimum Price', 'trim|integer');
			$this->form_validation->set_rules('maxprice', 'Maximum Price', 'trim|integer');
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$data = array(
					'mot_slug' => $this->_slug($this->sanitize_plain_text($this->input->post('mot_slug', FALSE), 120)),
					'dea_name' => $this->_slug($this->sanitize_plain_text($this->input->post('dea_name', FALSE), 120)),
					'km' => (int) $this->sanitize_phone_value($this->input->post('km', FALSE)),
					'minprice' => (int) $this->sanitize_phone_value($this->input->post('minprice', FALSE)),
					'maxprice' => (int) $this->sanitize_phone_value($this->input->post('maxprice', FALSE)),
				);
		...
		if($this->input->post('change_location')) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				//$slug, $lat, $long, $km, $loc_lat, $loc_long, $dealer, $minprice, $maxprice
				$data["lat"] = ($data["lat"] === '') ? 0 : $data["lat"] ; // if lat null
				$data["long"] = ($data["long"] === '') ? 0 : $data["long"] ; // if long null
				//redirect('motorcycles/dealers/' . $slug . '/' . $data['dem_colors'] . "/" . $data['lat'] . '/' . $data['long'] . '/' . 100 . '/' . 0 . '/' . 0 . '/' . 'dealer' . '/' . 0 . '/' . 0  ,'refresh');
	...
	public function _sort_dealers($slug, $dem_colors, $lat, $long, $km, $loc_lat, $loc_long, $dealer, $minprice, $maxprice,$promo) {
		...
		if($promo=='promo'){
    		...
    		$this->db->where('motorcycles.mot_slug', $slug);
    		$this->db->where('dealers_motorcycles.deb_id >=', 1);
    		$this->db->where('dealers_motorcycles.dem_status', 'published');
    		$this->db->where('motorcycles_pictures.mop_status', 'published');
    		$this->db->where('dealers_motorcycles.dem_promo ', 'true');
		...
		$this->db->where('motorcycles_pictures.mop_status', 'published');
		$this->db->where('dealers_motorcycles.dem_promo ', 'true');
    	$this->db->where('motorcycles.mot_promoname ', 'gms');
		$this->db->join("dealers_motorcycles", "dealers_motorcycles.deb_id = dealers_branches.deb_id");
		$this->db->join("dealers", "dealers.dea_id = dealers_branches.dea_id");
	...
	public function dev_info($slug) {
		...
		if($this->input->post('dealers_mode')) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$data = $this->sanitize_location_payload($this->input->post(NULL, FALSE));
				$data['lat'] = ($data['lat'] === '') ? 0 : $data['lat'];
				$data['long'] = ($data['long'] === '') ? 0 : $data['long'];

				// redirect('motorcycles/search/' . $data['mot_model'] . '/' . $data['mot_brand'] . '/' . $data['mot_type'] . '/' . $data['mot_transmission'] . '/' . $data['mot_diplacement'] . '/' . $data['mot_engine_type'] ,'refresh');

				//$slug, $lat, $long, $km, $loc_lat, $loc_long, $dealer, $minprice, $maxprice
				redirect('motorcycles/dealers/' . $slug . '/' . $data['dem_colors'] . "/" . $data['lat'] . '/' . $data['long'] . '/' . 100 . '/' . 0 . '/' . 0 . '/' . 'dealer' . '/' . 0 . '/' . 0 . '/' .'PROMO'   ,'refresh');
	...
	public function loadmotorebike(){
		$output = '';
		...
		foreach($data as $motorcycle)
			{
				$output .= '<a href=" '.base_url().'ebike'.'/'.strtolower($motorcycle['mot_brand']). '/' . $motorcycle['mot_slug'].' "></a>
				...
				<a href=" '.base_url().'ebike'.'/'.strtolower($motorcycle['mot_brand']). '/' . $motorcycle['mot_slug'].' ">
				...
				if($motorcycle['mot_brand']== "Yadea"){
					$output .= '<img class="brand-logo" src="'.base_url().'resources/site/newui-assets2/newlogomotor/yadea.jpeg"  width="50" height="30">';
				}
				$output .= '</div></div>';
			}
