...
$(document).on("click","#repost",function(){
    mot_id = $(this).data("mot_id");
if(confirm('Are you sure you want to repost this model for another 30 days? You can only repost it once.')){
    //window.location.href='<?php echo base_url()?>dealer/repo/repost'+ mot_id;
    window.open("<?php echo base_url()?>dealer/repo/repost/"+mot_id);
    location.reload();
}
});
</script>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php
                                            ...
                                            $converttime = strtotime($repos['mot_updated']);

                                            $datediff = $now - $converttime;
                                            $expired = round($datediff / (60 * 60 * 24));

                                            if ( $expired >= 30) {
                                                $label = 'label-danger';
                                                $word = "Expired";
                                            }elseif ( $repos['mot_status'] == 'published' ) {
                                                $label = 'label-success';
                                                $word = "approved";
                                        ...
                                        ?>
                                        <span class="label <?php echo $label;?>"><?php echo $word?></span>
                                    </td>
                                    <?php
                                    
                                    if($expired>=30 && $repos['mot_extend']==0 || $repos['mot_extend']==''){
                                         echo '<td><a href="javascript:;" id="repost" 
                                         data-mot_id="'.$repos['mot_id'].'">REPOST</a></td>';  
                                    }elseif($repos['mot_extend']==1){
                                    ...
                                    <td>    
                                        <div class="tools">
                                            ...
                                            <a href="<?php echo base_url() . 'dealer/repoq/edit/' . $repos['mot_id']?>"><i class="fa fa-edit"></i></a>
                                            <a href="<?php echo base_url() . 'dealer/repoq/delete/' . $repos['mot_id']?>"><i class="fa fa-trash-o"></i></a>
                                            <?php else: ?>
                                            <a href="<?php echo base_url() . 'dealer/repo/edit/' . $repos['mot_id']?>"><i class="fa fa-edit"></i></a>
                                            <a href="<?php echo base_url() . 'dealer/repo/delete/' . $repos['mot_id']?>"><i class="fa fa-trash-o"></i></a>
                                            <?php endif;?>
                                        </div>
                                    </td>
                                </tr>
