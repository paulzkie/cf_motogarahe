	...
	<script>
						...
						<div class="col-lg-9 col-sm-8 col-xs-12">
							<div class="row m-border">
								...
								<div class="col-lg-4 col-md-6 col-xs-12 zoomInUp" data-wow-delay="0.5s">
									<div class="b-items__cell results-motorcycle <?php foreach( $this->session->userdata('selected_dealers') as $selected_dealer ):?> <?php echo ( $selected_dealer == $motorcycle['dem_id'] ? 'selected_' : ''); ?> <?php endforeach;?>" href="<?php echo base_url('home')?>" id="<?php echo $motorcycle['deb_id']; ?>">
		...
		<div class="b-features">
			...
			<script type="text/javascript">
			    ...
			    // });
				function viewMap (id,lat,lng,brand){
						var db_id = id;
						var lats   = lat;
						var lngs   = lng;
						var brands = brand;
						var branch = $("#"+db_id+".b-items__cell").find(".brand_model").text();
...
$(document).ready(function(){
    ...
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
           ...
           var regions  = response.municipalities;
         // set all region
         regions.forEach(data =>  $('#muni').append($('<option>', {
                                            value: data.region_name,
                                            text: data.region_name})));
           $("#muni").change(function(){
            // to clear options of city per change
              $("#city").empty();
              // value of selected region
              var getVal = $(this).val();
              //  all cities selected
               var city =  regions.filter(function(data) {
                return data.region_name == getVal;
              });
               //  convertion start
             var cityArr = "";
             var result = city.map(data =>  data.cities.join() );
             var res1 = result.toString();
             var resArr = res1.split(",");
              resArr.forEach(data => $('#city').append($('<option>', { value: data,text: data}))   );
              cityArr == "";
              // convertion end
    ...
    xhttp.open("GET", "<?php echo base_url("uploads/json/")?>/city.json", true);
    xhttp.send();
